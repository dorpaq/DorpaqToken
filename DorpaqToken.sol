...
contract DPQDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SignedSafeMath for int256;
    ...
    constructor() DividendPayingToken("DPQ_Dividend_Tracker", "DPQ_Dividend_Tracker") {
        claimWait = 3600;
        minimumTokenBalanceForDividends = 10000 * (10**18); //must hold 10000+ tokens
    }

    function _transfer(address, address, uint256) internal pure override {
        require(false, "DPQ_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public pure override {
        require(false, "DPQ_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main DPQ contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 3600 && newClaimWait <= 86400, "DPQ_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");	
        require(newClaimWait != claimWait, "DPQ_Dividend_Tracker: Cannot update claimWait to same value");
...
2% of transaction fee is going to marketing wallet (in BNBs)
2% to LP pool and
4% to holders (in BNBs)
Total supply is 320,000,000,000

0xa090252d2795b891eb1cF5CEefefe6e49DA09b87 as marketing wallet and
0xE029d21653E925E1531A2905e9260F50805D176c as wallet named Team Dorpaq so tell me which one to use

*/

contract DORQToken is ERC20, Ownable, SafeToken, LockToken {
    using SafeMath for uint256;
    ...
    DPQDividendTracker public dividendTracker;

    uint256 public maxSellTransactionAmount = 320000000000 * (10**18);
    ...
    constructor() ERC20("DORPAQ", "DORQ") {
        BNBRewardsFee = 4;
        liquidityFee = 2;
        extraFeeOnSell = 0; // extra fee on sell
        marketingFee = 2;
        marketingWallet = payable(0xa090252d2795b891eb1cF5CEefefe6e49DA09b87);

        totalFees = BNBRewardsFee.add(liquidityFee).add(marketingFee); // total fee transfer and buy

    	dividendTracker = new DPQDividendTracker();
...
//0x9Ac64Cc6e4415144C455BD8E4837Fea55603e5c3
        ...
        */
        _mint(owner(), 320000000000 * (10**18));
    }
    ...
    function updateUniswapV2Router(address newAddress) public onlyOwner {
        require(newAddress != address(uniswapV2Router), "DPQ: The router already has that address");
        emit UpdateUniswapV2Router(newAddress, address(uniswapV2Router));
        uniswapV2Router = IUniswapV2Router02(newAddress);
    }

    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(_isExcludedFromFees[account] != excluded, "DPQ: Account is already the value of 'excluded'");
    ...
    function setAutomatedMarketMakerPair(address pair, bool value) public onlyOwner {
        require(pair != uniswapV2Pair, "DPQ: The PancakeSwap pair cannot be removed from automatedMarketMakerPairs");
    ...
    function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(automatedMarketMakerPairs[pair] != value, "DPQ: Automated market maker pair is already set to that value");
    ...
    function updateGasForProcessing(uint256 newValue) public onlyOwner {
        require(newValue >= 200000 && newValue <= 500000, "DPQ: gasForProcessing must be between 200,000 and 500,000");
        require(newValue != gasForProcessing, "DPQ: Cannot update gasForProcessing to same value");
